  ...
  namespace octree
  {
    ...
    class NBufferedBranchNode : public OctreeNode
    {
      ...
      public:
        ...
        {
          std::vector<OctreeNode*> child_node(8);
          child_node_array_.push_back(child_node);
          reset();
        }

        NBufferedBranchNode (unsigned short buffer_size) : OctreeNode()
        {
          expendBuffer(buffer_size);
          reset();
        }
        ...
        operator = (const NBufferedBranchNode &source_arg)
        {
          ...
          for (unsigned short b = 0; b < child_node_array_.size(); ++b)
        ...
        {
          assert( (buffer_arg<child_node_array_.size()) && (index_arg<8));
        ...
        void expendBuffer(unsigned short buffer_size)
        {
          while(child_node_array_.size() < buffer_size)
          {
            std::vector<OctreeNode*> child_node(8);
      ...
      protected:
        ...
        std::vector<std::vector<OctreeNode*>> child_node_array_;
    };
    ...
    class OctreeNBufBase
    {
      ...
      public:
        ...
        void
        nextBuffer () {
          switchBuffers(buffer_selector_ + 1);
        }

        void expendBufferRecursive (BranchNode* branch_arg, unsigned short buffer_size_new);
        ...
        void setBufferSize(unsigned short buffer_size_arg)
        {
          expendBufferRecursive(root_node_, buffer_size_arg);
          buffer_size_ = buffer_size_arg;
        }
      protected:
        ...
        {
          branch_arg.expendBuffer(buffer_size_);
          for(unsigned short selector = 0; selector < buffer_size_; selector++)
          {
            for(unsigned char child_idx = 0; child_idx < 8; child_idx++)
            {
              if(static_cast<BranchNode*>(new_child_arg)->hasChild(selector, child_idx))
              {
                branch_arg.setChildPtr(selector, child_idx_arg, new_child_arg);
                break;
              }
            }
          }
        ...
        inline  BranchNode* createBranchChild (BranchNode& branch_arg,
            unsigned char child_idx_arg)
        {
          BranchNode* new_branch_child = new BranchNode(buffer_size_);
            ...
            unsigned char child_idx_arg)
        {
          ...
          branch_arg.setChildPtr (buffer_selector_arg, child_idx_arg,
              static_cast<OctreeNode*> (new_branch_child));
        ...
        inline LeafNode*
        createLeafChild (BranchNode& branch_arg,
            unsigned short buffer_selector_arg,
            unsigned char child_idx_arg)
        {
          LeafNode* new_leaf_child = new LeafNode();

          branch_arg.setChildPtr(buffer_selector_arg, child_idx_arg, new_leaf_child);
        ...
        std::vector<bool> 
        getNodeBitPattern (const BranchNode& branch_arg,
                           unsigned char child_idx_arg);
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Globals
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        std::vector<std::size_t> leaf_count_all_;
  ...
  namespace octree
  {
    ...
    OctreeNBufBase<LeafContainerT, BranchContainerT>::deleteTree ()
    {
      if (root_node_)
      {
        ...
        for(unsigned short selector = 0; selector < buffer_size_; selector++)
        {
          treeCleanUpRecursive(root_node_, selector);
        }
        std::cout << "D" << std::endl;
        buffer_size_ = 0;
        std::cout << "E" << std::endl;
    ...
    {
      assert(buffer_selector_arg <= buffer_size_);
      if(buffer_selector_arg == buffer_size_)
      {
        expendBufferRecursive(root_node_, buffer_size_ + 1);
        buffer_size_++;
        leaf_count_all_.push_back(0);
      } else {
    ...
    template<typename LeafContainerT, typename BranchContainerT>
      ...
      if (depth_mask_arg > 1)
      {
        ...
        if (!branch_arg->hasChild(buffer_selector_, child_idx))
        {
          ...
          for(unsigned short selector = 0; selector < buffer_size_; selector++)
          {
            ...
            {
              child_branch = static_cast<BranchNode*> (branch_arg->getChildPtr(selector, child_idx));
              branch_arg->setChildPtr(buffer_selector_, child_idx, branch_arg->getChildPtr(selector, child_idx));
              do_create = false;
              doNodeReset = true;
              break;
            }
          ...
          if(do_create)
          {
            child_branch = createBranchChild (*branch_arg, buffer_selector_, child_idx);
            branch_count_++;
          }
        }
      ...
      else
      {
        ...
        if (!branch_arg->hasChild(buffer_selector_, child_idx))
        {
          child_leaf = createLeafChild (*branch_arg, buffer_selector_, child_idx);
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    template<typename LeafContainerT, typename BranchContainerT> void OctreeNBufBase<
        LeafContainerT, BranchContainerT>::expendBufferRecursive (BranchNode* branch_arg, unsigned short buffer_size_new)
    {
      branch_arg->expendBuffer(buffer_size_new);
      for (unsigned char child_idx = 0; child_idx < 8; child_idx++)
      {
        for(unsigned short selector = 0; selector < buffer_size_; selector++)
        {
          if (branch_arg->hasChild(selector, child_idx))
          {
            ...
            switch (child_node->getNodeType ())
            {
              ...
              {
                  // recursively proceed with indexed child branch
                  expendBufferRecursive (static_cast<BranchNode*> (child_node), buffer_size_new);
                  break;
              }
              case LEAF_NODE:
              {
          ...
          switch (child_node->getNodeType ())
          {
            case BRANCH_NODE:
              ...
              for(unsigned short selector = 0; selector < buffer_size_; selector++)
              {
                ...
                if(branch_arg->getChildPtr(buffer_selector_arg, child_idx) == branch_arg->getChildPtr(selector, child_idx))
                {
                  do_delete = false;
                  break;
                }
              ...
              if(do_delete) {
                std::cout << "AAAA " << buffer_selector_arg << std::endl;
                delete (child_node);
                std::cout << "B " << buffer_selector_arg << std::endl;
              }
              branch_arg->setChildPtr(buffer_selector_arg, child_idx, nullptr);
              break;
            case LEAF_NODE:
            
              std::cout << "CCCC " << buffer_selector_arg << std::endl;
              delete (child_node);
              std::cout << "D " << buffer_selector_arg << std::endl;
              branch_arg->setChildPtr(buffer_selector_arg, child_idx, nullptr);
